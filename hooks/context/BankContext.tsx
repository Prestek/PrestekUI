...
export const BankProvider: React.FC<ChildrenProps> = ({ children }) => {
    ...
    try {
      ...
      setTotalPending(
        orderedApplications.filter(
          (a: Application) => a.status === LoanRequestStatus.PENDING
        ).length
      );
      setTotalApproved(
        orderedApplications.filter(
          (a: Application) => a.status === LoanRequestStatus.APPROVED
        ).length
      );
      setTotalRejected(
        orderedApplications.filter(
          (a: Application) => a.status === LoanRequestStatus.REJECTED
        ).length
      );
    } catch (error) {
      console.error(error);
  ...
  const addApplication = (newApplication: Application) => {
    const lastApplication = applications.find(
      (a: Application) => a.id === newApplication.id
    );
    if (lastApplication) {
      const newApplications = applications.splice(
        applications.indexOf(lastApplication),
        1,
        newApplication
      );
